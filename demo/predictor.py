...
from fastreid.engine.defaults import LATexPredictor
...
class FeatureExtractionLATex(object):
    def __init__(self, cfg, parallel=False):
        ...
        self.predictor = LATexPredictor(cfg)
    ...
    def run_on_loader(self, data_loader):
        if self.parallel:
            ...
            for batch in data_loader:
                predictions = self.predictor(batch["images"], batch["camids"], batch["viewids"],batch['attributes'], batch["targets"])
                yield predictions, batch["targets"].cpu().numpy(), batch["camids"].cpu().numpy()
