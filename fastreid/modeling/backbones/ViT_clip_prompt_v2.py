...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int,
        ...
        self.width = width
        prompt_list = [nn.Parameter(torch.zeros(1, self.b, width)) for _ in range(layers - 1)]

        self.prompt = nn.ParameterList(prompt_list).cuda()

        for prompt in self.prompt:
            trunc_normal_(prompt, std=.02)

        self.pt2 = nn.Parameter(torch.zeros(1, 15, width))
        trunc_normal_(self.pt2, std=.02)

        self.lock_net(self.transformer)
        self.lock_net(self.conv1)
        self.lock_net(self.ln_pre)
        self.lock_net(self.ln_post)
    ...
    def forward(self, x: torch.Tensor, camid):
        ...
        # 前11层
        for bk, p in zip(self.transformer.resblocks[:11], self.prompt[:11]):
            B = x.shape[1]
            p = p.permute(1, 0, 2)
            p_expand = p.expand(-1, B, -1)
            x = torch.cat((x, p_expand), dim=0)
            x = bk(x)
        ...
        p = self.pt2.permute(1, 0, 2)

        p_expand = p.expand(-1, B, -1)
        x = torch.cat((x, p_expand), dim=0)
        x = self.transformer.resblocks[11](x)

        p_instance = x[-15:, :, :]
        x = x[:-15, :, :]

        p_instance = p_instance.permute(1, 0, 2)
        x = x.permute(1, 0, 2)
        if self.proj is not None:
            # xproj = x12 @ self.proj
            xproj = x @ self.proj
            p_instance = p_instance @ self.proj

        return xproj[:, 0].unsqueeze(-1).unsqueeze(-1), p_instance, xproj


class Text_encoder(nn.Module):
    def __init__(self, token_embedding, positional_embedding, transformer, ln_final, text_projection, dtype):
        super().__init__()
...
class CLIP(nn.Module):
    ...
    def create_text(self):
        self.text_encoder = Text_encoder(self.token_embedding, self.positional_embedding, self.transformer,
                                         self.ln_final,
                                         self.text_projection, self.dtype)
